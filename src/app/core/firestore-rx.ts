      ...
      retry({
        ...
        delay: (error: FirestoreError, retryCount: number) => {
          ...
          if (retryCount >= maxRetries) {
            console.error(
              `[firestore] permission-denied não resolveu após ${maxRetries} tentativas — provavelmente não é uma corrida transitória, é uma regra realmente negando (ex: doc /users/{uid} com ID diferente do UID de auth).`,
              error,
            );
          } else {
